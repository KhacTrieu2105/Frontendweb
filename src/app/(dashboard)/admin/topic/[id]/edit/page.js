...
export default function EditTopicPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await TopicService.update(id, form);
      ...
      if (res && res.status === true) {
        ...
        setError("Cập nhật chủ đề thất bại. Vui lòng thử lại.");
      }
    } catch (err) {
      console.error("Update topic error:", err);
      setError("Cập nhật chủ đề thất bại. Vui lòng kiểm tra lại thông tin.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!form) {
    return (
      <div className="p-6 min-h-screen bg-[#F8F9FA] font-sans flex items-center justify-center">
        <div className="text-center font-medium text-gray-500">Đang tải thông tin chủ đề...</div>
      </div>
    );
  }
      ...
      <div className="max-w-5xl mx-auto space-y-8">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-[#1E1E2D] tracking-tight">
                SỬA <span className="text-rose-500">CHỦ ĐỀ</span>
              </h1>
              <p className="text-gray-500 font-medium mt-1">
                Chỉnh sửa thông tin chủ đề #{id}.
              </p>
            </div>
          </div>
        </div>
        ...
        {success && (
          <div className="bg-green-50 border border-green-200 rounded-2xl p-8 text-center">
            ...
            <h3 className="text-xl font-bold text-green-800 mb-2">Cập nhật thành công!</h3>
            <p className="text-green-700">Chủ đề đã được cập nhật thành công.</p>
          </div>
        )}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-50 p-8">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xl font-black text-gray-900">Cài đặt</h3>
              </div>
